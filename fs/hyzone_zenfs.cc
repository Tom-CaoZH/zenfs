/**
 * @file hyzone_zenfs.cc
 * @brief Hyzone Block Device for ZenFS
*/

#if !defined(ROCKSDB_LITE) && !defined(OS_WIN)

#include "hyzone_zenfs.h"
...
Hyzone::Hyzone(HyzonedBlockDevice *zbd, struct logical_zone *z)
    : zbd_(zbd),
      busy_(false),
      start_(z->start_address),
      max_capacity_(z->capacity),
      wp_(zbd_zone_wp(z)) {
...
bool Hyzone::IsUsed() { return (used_capacity_ > 0); }
uint64_t Hyzone::GetCapacityLeft() { return capacity_; }
bool Hyzone::IsFull() { return (capacity_ == 0); }
bool Hyzone::IsEmpty() { return (wp_ == start_); }
uint64_t Hyzone::GetZoneNr() { return start_ / zbd_->GetZoneSize(); }

void Hyzone::EncodeJson(std::ostream &json_stream) {
  json_stream << "{";
...
IOStatus Hyzone::Reset() {
  size_t zone_sz = zbd_->GetZoneSize();
  unsigned int report = 1;
  struct logical_zone z;
  int ret;
...
IOStatus Hyzone::Append(char *data, uint32_t size) {
...
inline IOStatus Hyzone::CheckRelease() {
  if (!Release()) {
    assert(false);
...
Hyzone *HyzonedBlockDevice::GetIOZone(uint64_t offset) {
  for (const auto z : io_zones)
    if (z->start_ <= offset && offset < (z->start_ + zone_sz_)) return z;
  return nullptr;
}

HyzonedBlockDevice::HyzonedBlockDevice(std::string bdevname,
...
std::string HyzonedBlockDevice::ErrorToString(int err) {
  char *err_str = strerror(err);
  if (err_str != nullptr) return std::string(err_str);
  return "";
}

IOStatus HyzonedBlockDevice::CheckScheduler() {
...
IOStatus HyzonedBlockDevice::Open(bool readonly, bool exclusive) {
  struct logical_zone *zone_rep;
  ...
  while (m < ZENFS_META_ZONES && i < reported_zones) {
    struct logical_zone *z = &zone_rep[i++];
    /* Only use sequential write required zones */
    if (zbd_zone_type(z) == ZBD_ZONE_TYPE_SWR) {
      if (!zbd_zone_offline(z)) {
        meta_zones.push_back(new Hyzone(this, z));
      }
      m++;
    }
  }
  ...
  for (; i < reported_zones; i++) {
    ...
    if (zbd_zone_type(z) == ZBD_ZONE_TYPE_SWR) {
      if (!zbd_zone_offline(z)) {
        Hyzone *newZone = new Hyzone(this, z);
        if (!newZone->Acquire()) {
          assert(false);
...
void HyzonedBlockDevice::LogZoneStats() {
...
void HyzonedBlockDevice::LogZoneUsage() {
  for (const auto z : io_zones) {
...
void HyzonedBlockDevice::LogGarbageInfo() {
...
HyzonedBlockDevice::~HyzonedBlockDevice() {
...
IOStatus HyzonedBlockDevice::AllocateMetaZone(Hyzone **out_meta_zone) {
...
IOStatus HyzonedBlockDevice::ResetUnusedIOZones() {
  for (const auto z : io_zones) {
    if (z->Acquire()) {
...
void HyzonedBlockDevice::WaitForOpenIOZoneToken(bool prioritized) {
...
bool HyzonedBlockDevice::GetActiveIOZoneTokenIfAvailable() {
...
void HyzonedBlockDevice::PutOpenIOZoneToken() {
  {
    std::unique_lock<std::mutex> lk(zone_resources_mtx_);
    open_io_zones_--;
  }
...
void HyzonedBlockDevice::PutActiveIOZoneToken() {
  {
    std::unique_lock<std::mutex> lk(zone_resources_mtx_);
    active_io_zones_--;
  }
...
IOStatus HyzonedBlockDevice::ApplyFinishThreshold() {
  IOStatus s;
...
IOStatus HyzonedBlockDevice::FinishCheapestIOZone() {
  IOStatus s;
  Hyzone *finish_victim = nullptr;
...
IOStatus HyzonedBlockDevice::GetBestOpenZoneMatch(
    Env::WriteLifeTimeHint file_lifetime, unsigned int *best_diff_out,
    Hyzone **zone_out, uint32_t min_capacity) {
  unsigned int best_diff = LIFETIME_DIFF_NOT_GOOD;
  Hyzone *allocated_zone = nullptr;
  IOStatus s;
...
IOStatus HyzonedBlockDevice::AllocateEmptyZone(Hyzone **zone_out) {
  IOStatus s;
  Hyzone *allocated_zone = nullptr;
  for (const auto z : io_zones) {
    if (z->Acquire()) {
...
int HyzonedBlockDevice::DirectRead(char *buf, uint64_t offset, int n) {
  int ret = 0;
  int left = n;
  int r = -1;
...
IOStatus HyzonedBlockDevice::ReleaseMigrateZone(Hyzone *zone) {
...
IOStatus HyzonedBlockDevice::TakeMigrateZone(Hyzone **out_zone,
                                           Env::WriteLifeTimeHint file_lifetime,
...
IOStatus HyzonedBlockDevice::AllocateIOZone(Env::WriteLifeTimeHint file_lifetime,
                                          IOType io_type, Hyzone **out_zone) {
  Hyzone *allocated_zone = nullptr;
  unsigned int best_diff = LIFETIME_DIFF_NOT_GOOD;
  int new_zone = 0;
  IOStatus s;
...
std::string HyzonedBlockDevice::GetFilename() { return filename_; }

uint32_t HyzonedBlockDevice::GetBlockSize() { return block_sz_; }

void HyzonedBlockDevice::EncodeJsonZone(std::ostream &json_stream,
                                      const std::vector<Hyzone *> zones) {
  bool first_element = true;
  json_stream << "[";
  for (Hyzone *zone : zones) {
    if (first_element) {
      first_element = false;
    } else {
      json_stream << ",";
...
void HyzonedBlockDevice::EncodeJson(std::ostream &json_stream) {
  json_stream << "{";
  json_stream << "\"meta\":";
...
IOStatus HyzonedBlockDevice::GetZoneDeferredStatus() {
  std::lock_guard<std::mutex> lock(zone_deferred_status_mutex_);
  return zone_deferred_status_;
}

void HyzonedBlockDevice::SetZoneDeferredStatus(IOStatus status) {
...
void HyzonedBlockDevice::GetZoneSnapshot(std::vector<ZoneSnapshot> &snapshot) {
